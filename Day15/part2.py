	...
	for i in range(5):
		for j in range(len(square)):
			for k in range(len(square[0])):
				if square[j][k] + i > 9:
					erg[j].append(square[j][k] + i - 9)
				else:
					erg[j].append(square[j][k] + i)

	for i in range(1, 5):
		for j in range(len(square)):
			erg.append(erg[j].copy())
			last = len(erg) - 1
			for l in range(len(erg[last])):
				erg[last][l] = erg[last][l] + i
				if erg[last][l] > 9:
					erg[last][l] -= 9
...
if __name__ == '__main__':
	...
	field = createField(file)
	printList(field)
	printList(graphentheorie(field, 0, 0))
